...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        print("isLoogedin -- > \(UserDefaults.standard.value(forKey: "isLoggedIn") as? Bool ?? false)")
        if UserDefaults.standard.value(forKey: "isLoggedIn") as? Bool ?? false {
            initialViewController = mainStoryboard.instantiateViewController(withIdentifier: "ListUsersViewController") as! ListUsersViewController
        } else {
